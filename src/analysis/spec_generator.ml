...
let generate_spec_statesEqual (em_vars : (ty binding * ety) list) : sexp =
  ...
  (* realWorld I/O model variables: always equal across commutativity *)
  let rw_eqs = List.map (fun n ->
    EBop (Eq, EVar (Var n), EVar (VarPost n))
  ) realWorld_vars in

  sexp_of_sexp_list ([alloc_eq] @ value_eqs @ next_eqs @ loc_arr_eqs @ other_eqs @ rw_eqs)
...
let get_postconditions () : sexp =
  ...
  Hashtbl.iter (fun key -> fun value -> 
                if List.mem key heap_model_vars || List.mem key realWorld_vars then exp_list := !exp_list @ [final_mangle_id !value key]
                else
...
let rec exp_to_smt_exp (e: exp node) (side: int) ?(indexed = true) (vctrs : (string, int ref) Hashtbl.t) : sexp * sexp Smt.bindlist =
    ...
    | Call (MethodL (id, {pc=None; _}), el) ->
      let args_rtn, args_binds = List.split @@ List.map (fun exp -> exp_to_smt_exp exp right ~indexed vctrs) el in
      let args_types = List.map (fun e -> sty_of_ty (infer_gstates_type e)) el in
      let smt_fn = { name = id ; args= args_types; ret = Smt.TInt} in
